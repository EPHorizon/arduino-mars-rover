...
class Console(Frame):
    ...
    def __init__(self):
        #self.port = serial.Serial("COM12", 57600, timeout=2)
    ...
    def getCenter(self, ct):
        ...
        self.maxBlue = 0
        self.blueLoc = [0,0]
        for x in range(ct[0]-125, ct[0]+125): #localize the search area to 250x250
            for y in range(ct[1]-125, ct[1]+125):
                if ct[0]+125 > 1600 or ct[1]+125 > 1200: #ensure the box is valid
        ...
        #Check if the new values differ significantly (which implies a bad calculation)
        if abs(center[0] - oldCenter[0]) > 150 or abs(center[1] - oldCenter[1]) > 150:
            self.firstImage()
            return center
    ...
    def boundCheck(self, ct):
        ...
        self.boundary = Image.open(r"C:\Users\Robert Davis\Documents\GitHub\arduino-mars-rover\boundary.png")
        ...
        if self.bound[ct[0], ct[1]] == (255, 255, 255, 255):
            ...
            if self.theta_1st < -180:
                self.theta_1st = self.theta_1st + 360
            elif self.theta_1st > 180:
                self.theta_1st = self.theta_1st - 360
            ...
            if self.theta_2nd < -180:
                self.theta_2nd = self.theta_2nd + 360
            elif self.theta_2nd > 180:
                self.theta_2nd = self.theta_2nd - 360
